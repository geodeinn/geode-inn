...
- Does not speak to the player unless the player sits down beside Scarlett. The both are: the adequate — the approach and the wait, the same trust, different pace.
- When the player sits, Scarlett shows the player a drawing — a picture of the dragon as Scarlett sees it. The drawing is the player's first glimpse of the dragon's true form (not the game sprite, but the dragon as a child sees it — rounder, softer, more alive). The both are: the adequate — the game dragon and the drawn dragon, the same creature, different eye.
- At Whisper Layer Tier 3, Scarlett's drawing changes — the dragon in the drawing is looking at the player. The dragon in the game is not. The drawing knows something the game doesn't. The both are: the adequate — the art and the system, the same dragon, different awareness.

**Scarlett's quest connection:** Scarlett is the NPC who reveals the dragon's true name. The dragon's name is not spoken — it's drawn. Scarlett draws the name in the sand beside the dragon, and the sand holds the shape for a moment before the 42Hz hum dissolves it. The player who sees the name before it fades earns the dragon's trust. The both are: the adequate — the word and the picture, the same name, different language.

**Scarlett's dialogue voice:** Barely above a whisper. Three words at a time, maximum. The both are: the adequate — the volume and the meaning, the same voice, different weight. Every word Scarlett says matters because Scarlett says so few.

**Full profile exists:** docs/geode_inn_npc_scarlett.md (referenced in master structure as complete)
...
- The child's structures are the Inn's building system in miniature — the child is the building system's living tutorial. The player who watches the child build learns the system's logic. The both are: the adequate — the tutorial and the child, the same lesson, different teacher.
- The child's structures occasionally vibrate at 42Hz — the child builds in tune with the Inn. The structures are resonant. The both are: the adequate — the building and the song, the same material, different frequency.
- At Whisper Layer Tier 2, the child's structures begin to match the player's recent builds — the child is watching the player build too. The both are: the adequate — the watcher and the builder, the same observation, different direction.

**Dialogue voice:** Enthusiastic, fast, interrupted by the child's own ideas. "Look at this — no wait, look at THIS — I put the quartz on top and it HUMS — does yours hum? Let me see yours." The both are: the adequate — the excitement and the craft, the same builder, different age.

**Quest connection:** No formal quest — this child is a LIVING TUTORIAL. The child teaches the building system through play. The player who builds beside the child learns faster than the player who reads the instructions. The both are: the adequate — the play and the lesson, the same system, different pace.
...
**Presence in the game:**
- Found in various zones, always holding something the player hasn't seen before — a stone, a shell, a feather, a shard of pottery. The both are: the adequate — the found and the new, the same object, different hand.
- The youngest child gives found objects to the player freely — "Here. I found this." No cost, no quest, no exchange. The both are: the adequate — the gift and the free, the same generosity, different scale. The youngest child is the Inn's purest gift economy — the child who gives because giving is what children do.
- The found objects are occasionally significant — a stone the player needs, a key to a puzzle, a charm the player hasn't earned yet. The both are: the adequate — the child and the shortcut, the same path, different door.
- At Whisper Layer Tier 4, the youngest child finds something that shouldn't exist: a small stone with the player's name carved in it. The child doesn't know the name. The child just found it. The both are: the adequate — the found and the impossible, the same stone, different maker. The stone is the Inn's most direct Whisper Layer statement — the Inn carved the player's name and let the youngest child find it. The both are: the adequate — the archive and the child, the same message, different messenger.

**Dialogue voice:** Simple, direct, joyful. "Look what I found!" is the most common line. The both are: the adequate — the simple and the profound, the same discovery, different vocabulary.
...
- **The Living Book** — the four children are the Living Book's future readers — the generation who will read the archive after the current player is gone.

---

## Names Pending

Two of the four children (the second son, age 14, and the youngest, age 9) need names from Krista. The both are: the adequate — the known and the waiting, the same family, different page. The Inn is patient. The names will come when they come.
